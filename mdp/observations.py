...
def eef_pose(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
...
def pose_distance(env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    distance_pose = torch.cat((distance_xyz, distance_ang), 1)
    
    return distance_pose
...
def contact_force(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    contact_force = net_contact_forces[:, :, sensor_cfg.body_ids]
    # is_contact = torch.max(torch.norm(net_contact_forces[:, :, sensor_cfg.body_ids], dim=-1), dim=1)[0] > threshold
    # sum over contacts for each environment
    # print("contact_force.squeeze()", contact_force.squeeze())
    return contact_force.squeeze()

# def generated_commands(env: ManagerBasedRLEnv, command_name: str) -> torch.Tensor:
#     """The generated command from command term in the command manager with the given name."""
#     print("observations", env.command_manager.get_command(command_name))
